...
#implicitily_wait是用来判断整个网页是否还没有创建好，所以需要time.sleep()提高程序的稳定性
time.sleep(2)
#driver.find_element_by_css_selector("#filePicker label").click()
#因为真正负责上传文件的页面元素<input type="file"...>
#所以我们可以直接操作这个控件
#这个控件可以直接输入图片的路径
...
#range(10)表示0到9，一共10个数字，range是区间的意思，默认从0开始，到长度减1结束
...
#check=driver.find_element_by_link_text("查看")
#driver.execute_script('arguments[0].removeAttribute("target")',check)#删除
#check.clear()
#driver.find_element_by_class_name("goods-num-add").click()
driver.execute_script("window.scrollTo(200,100)")